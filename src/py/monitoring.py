...
class Signals:
	...
	def __init__(self):
		...
		self.onShutdown = []
		self.hasSignalModule = signal is not None
...
class Logger:
	...
	@classmethod
	def Err(cls, *message):
		cls.Instance().err(*message)

	@classmethod
	def Warn(cls, *message):
		cls.Instance().warn(*message)
	...
	@classmethod
	def Traceback(cls):
		cls.Instance().traceback()

	@classmethod
	def Output(cls, *message):
		cls.Instance().output(*message)
...
class Log(Action):
	...
	def getMessage(self):
		...
		if isinstance(self.message, types.LambdaType):
			message = message()
		return message
